...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ---- Desktop "Regions" dropdown: smooth hover with close-delay ----
  // Prevents the menu from vanishing the instant the cursor leaves the
  // toggle link (e.g. while moving diagonally down toward the submenu).
  // The dropdown opens instantly, but only closes after a short grace
  // period, and that timer is cancelled if the cursor re-enters in time.
  document.querySelectorAll('.nav-dropdown').forEach((dropdown) => {
    let closeTimer = null;
    const DROPDOWN_CLOSE_DELAY = 350; // ms
  ...
  // ---- Mobile menu toggle (slide-in drawer + dark overlay) ----
  ...
  // Close on overlay click (tap outside the drawer)
  navOverlay?.addEventListener('click', closeMenu);

  // Close on Escape key
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && navToggle?.classList.contains('open')) closeMenu();
  });
